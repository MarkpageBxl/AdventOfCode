...
# Day 7, part 2
#
# Challenge URL: https://adventofcode.com/2023/day/7#part2
#
...
def generate_joker_hands(hand: str, joker_positions: list[int]) -> Iterable[str]:
    product = itertools.product(CARD_ORDER[1:], repeat=len(joker_positions))
    for p in product:
        mod_hand = list(hand)
        for i, joker_position in enumerate(joker_positions):
            mod_hand[joker_position] = p[i]
        yield "".join(mod_hand)


def hand_type_strength_jokers(hand: str) -> int:
    joker_positions = [i for i, _ in enumerate(hand) if hand[i] == "J"]
    if len(joker_positions) == 0:
        return hand_type_strength(hand)
    best_hand = 0
    for hand in generate_joker_hands(hand, joker_positions):
        hand_value = hand_type_strength(hand)
...
def hand_sort_key(hand: str):
    key = [hand_type_strength_jokers(hand)]
